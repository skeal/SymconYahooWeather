	...
	private function GenerateWeatherTable($Value){
    	...
    	if( $Value->query->count > 0 ){
			...
			if ($forecast[0]->code == '0') $vorhersage_heute .= 'Tornado'; 
				...
				if ($forecast[0]->code == '15') $vorhersage_heute .= 'Stürmiger Schneefall'; 
				...
				if ($forecast[0]->code == '23') $vorhersage_heute .= 'Stürmisch'; 
				if ($forecast[0]->code == '24') $vorhersage_heute .= 'Windig'; 
				if ($forecast[0]->code == '25') $vorhersage_heute .= 'Kalt'; 
				if ($forecast[0]->code == '26') $vorhersage_heute .= 'Bewölkt'; 
				if ($forecast[0]->code == '27') $vorhersage_heute .= 'Größtenteils bewölkt<br>(nachts)'; 
				if ($forecast[0]->code == '28') $vorhersage_heute .= 'Größtenteils bewölkt<br>(tagsüber)'; 
				if ($forecast[0]->code == '29') $vorhersage_heute .= 'Teilweise bewölkt (nachts)'; 
				if ($forecast[0]->code == '30') $vorhersage_heute .= 'Teilweise bewölkt (tagsüber)'; 
				if ($forecast[0]->code == '31') $vorhersage_heute .= 'Klar (nachts)'; 
				if ($forecast[0]->code == '32') $vorhersage_heute .= 'Sonnig'; 
				if ($forecast[0]->code == '33') $vorhersage_heute .= 'Schön (nachts)'; 
				if ($forecast[0]->code == '34') $vorhersage_heute .= 'Schön (tagsüber)'; 
				if ($forecast[0]->code == '35') $vorhersage_heute .= 'Regen und Hagel'; 
				if ($forecast[0]->code == '36') $vorhersage_heute .= 'Heiß'; 
				...
				if ($forecast[0]->code == '44') $vorhersage_heute .= 'Teilweise bewölkt'; 
			...
			for( $i = 0; $i < $this->ReadPropertyInteger("YWHDays"); $i++ ){
				$weatherstring .= '<td align="center">';
				$weatherstring .= '<img src="/hook/SymconYahooWeather/' .$forecast[$i]->code .'.png" style="height:' .$this->ReadPropertyInteger("YWHImageZoom") .'%;width: ' .$this->ReadPropertyInteger("YWHImageZoom") .'%;">';
